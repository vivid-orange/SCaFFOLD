...
public abstract class CalcValue<T> : ICalcValue, IEquatable<CalcValue<T>>
{
    ...
    {
        string name = string.IsNullOrEmpty(x.EntityLabel) || string.IsNullOrEmpty(y.EntityLabel)
            ? string.Empty : $"{x.EntityLabel}\u2009{operation}\u2009{y.EntityLabel}";
    ...
    internal static void CheckUnitsAreTheSame<U1, U2>(CalcValue<U1> x, CalcValue<U2> y)
    {
        if (x.Unit != y.Unit)
        {
            throw new UnitsNotSameException(x.EntityLabel, y.EntityLabel, x.Unit, y.Unit);
        }
    }
    ...
    {
        return EntityLabel.GetHashCode() ^ Symbol.GetHashCode() ^ Status.GetHashCode()
