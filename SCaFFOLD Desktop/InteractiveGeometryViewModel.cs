...
public class InteractiveGeometryViewModel : ViewModelBase
{
    ...
    private void InitializeGeometry()
    {
        ...
        {
            foreach (IInteractiveGeometryItem? item in _model.InteractiveGeometryItems)
            {
        ...
        {
            foreach (GeometryBase? geo in _model.Geometry)
            {
                AddGeometry(geo);
            }
        }
    ...
    private void AddGeometry(GeometryBase geo)
    {
        ...
        else if (geo is PolyLine poly)
        {
            ...
            {
                foreach (GeometryBase? segment in poly.Segments)
                {
                    AddGeometry(segment);
                }
            }
    ...
    public void Refresh()
    {
        ...
        // 2. Refresh Points
        foreach (GeometryPointViewModel point in Points)
        {
    ...
    {
        if (Points.Count == 0 && StaticGeometry.Count == 0)
        {
            return;
        }
        ...
        foreach (ViewModelBase item in StaticGeometry)
        {
    ...
    {
        foreach (GeometryPointViewModel p in Points)
        {
            p.SetTransform(_scaleX, _scaleY, _offsetX, _offsetY);
        }
        ...
        foreach (ViewModelBase item in StaticGeometry)
        {
            if (item is GeometryLineViewModel l)
            {
                l.SetTransform(_scaleX, _scaleY, _offsetX, _offsetY);
            }
            else if (item is GeometryArcViewModel a)
            {
                a.SetTransform(_scaleX, _scaleY, _offsetX, _offsetY);
            }
        }
    }
